...
class MusicEmotionRecommender:
    def recommend_music(self, user_emotion, music_data, mode='maintain', top_n=3, user=None):
        ...
        # 1. '싫어요' 한 음악은 후보군에서 원천 제외 (Hard Filtering)
        if user and user.is_authenticated:
            from daybydaybackend.diary.models import UserFeedback
            disliked_ids = set(UserFeedback.objects.filter(user=user, is_like=False, music__isnull=False).values_list('music__id', flat=True))
            music_data = [t for t in music_data if t.get('track_id') not in disliked_ids]
