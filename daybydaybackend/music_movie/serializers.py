...
class MovieResponseSerializer(serializers.Serializer):
    ...
    def get_movie_id(self, obj):
        if hasattr(obj, 'tmdb_id'):
            return obj.tmdb_id
        return obj.get('movie_id')

    def get_director(self, obj):
        if hasattr(obj, 'director'):
            return obj.director if obj.director else ''
        return obj.get('director', '')

    def get_image_url(self, obj):
        if hasattr(obj, 'poster_path'):
            return obj.poster_path if obj.poster_path else ''
        return obj.get('image_url', '')
